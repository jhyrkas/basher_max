...
static t_class *protractor_class;
...
void send_output(t_protractor *x, int change) {
    // output pan
    for (int i = 0; i < x->limit; i++) {
...
void set_new_freqs(t_protractor *x, t_symbol *s, long argc, t_atom *argv) {
...
void set_uniform_spread(t_protractor *x, long s) {
    // don't change pan
    if (s < 1) {
    ...
    // initial pan: spread frequencies in circle (initial start where??? this seems to work okay so i'm trying it)
    if (s == 1) {
...
// constructor
static void *protractor_new(long n)
{
    t_protractor *x = (t_protractor *)object_alloc(protractor_class); // create new instance
...
{
    ps_list = gensym("list");
    protractor_class = class_new(
            "protractor", // class name
            (method)protractor_new, // constructor
            (method)NULL, // destructor
    	    sizeof(t_protractor), // class size in bytes
            0L, // graphical representation, depr
            A_DEFLONG, // initial spread
            0 // default value
    );
    class_addmethod(protractor_class, (method)opt_step, "bang", 0);
    class_addmethod(protractor_class, (method)set_uniform_spread, "int", A_LONG, 0);
    class_addmethod(protractor_class, (method)set_new_freqs, "list", A_GIMME, 0);
    class_addmethod(protractor_class, (method)set_more_diss, "in1", A_LONG, 0);
    class_register(CLASS_BOX, protractor_class);
}
